...
function navBorderBottom () {
  ...
  $('nav').css('border-bottom', '10px solid rgb(180,40,40)')
  return $()
  // return $()
} 
...
function evenBoxesText () {
  ...
  $('.box:nth-child(even)').text('I am a box')
  return '';
}
...
function oddBoxesHtml () {
  // set the HTML content of the odd boxes to "<div> I am an inner box</div>"
  $('.box:nth-child(odd)').text('I am an inner box')
  return '';
}
...
function modifyNav ()  {
  const text = $('h1').text()
  $('h1').text(text.replace('Your Name', 'Alicia'))
  return '';
}
